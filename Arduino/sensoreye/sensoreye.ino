...
void setup() {
  if(debug){
    ...
    Serial.println("Versao: 3.1 LAMBDA");
...
void linguagem(int distancia) {
  ...
  /////////////////////////////////////////
  int intensidade = map(cm, 240, 0, 100, 255);
  int delayintensidade = map(cm, 240, 0, 500, 1200);
  
  if (cm > 0 && cm < 240) {
     vibracall(intensidade, delayintensidade, delayintensidade); 
  }
